...
export default {
    ...
    methods:{
        getOrderInfoData(){
            this.$axios.get(`site/validate/order/getorderdetial/${this.$route.query.orderid}`).then(res=>{
            // console.log(res.data.message.orderinfo);
            //商品列表
            this.goodslist=res.data.message.goodslist
            //订单信息
            this.orderinfo=res.data.message.orderinfo
            // 商品总价格
            res.data.message.goodslist.forEach(item => {
                this.totalPrice+=item.quantity*item.real_price
            });
            //商品总价格
